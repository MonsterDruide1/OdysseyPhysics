...
bool CollisionMultiShape::check(CollisionShapeKeeper* keeper, const sead::Matrix34f* mtx,
    ...
    printf("CollisionMultiShapeSimple::check((%.02f,%.02f,%.02f,%.02f,%.02f,%.02f,%.02f,%.02f,%.02f,%.02f,%.02f,%.02f), %.02f, (%.02f,%.02f,%.02f)) => %d\n",
           ...
           val, vec.x, vec.y, vec.z, mCollisionShapeKeeper->mNumCollideResult);
  ...
  if ( mPtrNum >= 1 )
  {
    ...
    while ( 1 )
    {
      ...
      if ( CollisionShapeFunction::isShapeDisk(v9) )
      {
        ...
        LOBYTE(v13) = sqrtf(
                        (float)((float)(v21 + (float)(v24 * v26)) * (float)(v21 + (float)(v24 * v26)))
                      + (float)((float)((float)(v19 + (float)(v23 * v26)) * (float)(v19 + (float)(v23 * v26)))
                              + (float)((float)(v20 + (float)(v22 * v26)) * (float)(v20 + (float)(v22 * v26))))) > (float)(mBoundingSphereRange + ShapeInfoDisk[11]);
        */
      }
      else
      {
    ...
    if(mCollisionShapeKeeper->isShapeArrow(mCurrentShapeIndex)) {
      ...
      /*if ( v12->someCounter && !v12->mIsMoving
          ...
          || (v13 = al::Triangle::getFaceNormal(&info.mTriangle),
              (float)((float)((float)(v13->x * this->unk3.x) + (float)(v13->y * this->unk3.y)) + (float)(v13->z * this->unk3.z)) <= 0.0) )
      */
      if(false) return;
      ...
      const CollisionShapeInfoSphere* sphere = mCollisionShapeKeeper->getShapeInfoSphere(mCurrentShapeIndex);
